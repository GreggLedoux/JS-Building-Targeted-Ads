...
async function getCoords() {
    ...
    return [position.coords.latitude, position.coord.longitude]
}
...
function getMealTime() {
    const hour = userTime()
    return hour > 20 ? 'late-night sanck' : hour > 16 ? 'dinner': hour >11  ? 'lunch' : 'breakfast'
}
...
function buildAd2(coords) {
    const mapLink = `https//www.google,com/mapps/search/coffee/@${coords[0]}.${coords[1]}`
    const content = document.querySelector('.ad2')
    const inner = document.createElelment('p')
